define([
    ...
    function updateTile(tileset, tile, frameState) {
        var parent = tile.parent;
        var parentTransorm = define(parent) ? parent.computedTransform : tileset._modelMatrix;
    ...
    function shouldVisit(tileset, tile, screenSpaceError, frameState) {
        // Not visible
        ...
        if (!meetsScreenSpaceError || useOptimization) {
    ...
    function shouldTraverse(tileset, tile, screenSpaceError, frameState) {
        // TODO : why?
        // For additive refinement - use parent's geometric error with child's box to see if we already meet the SSE
        if ((tile.refine === Cesium3DTileRefine.ADD) && defined(tile.parent)) {
            return getScreenSpaceError(tileset, tile.parent.geometricError, tile, frameState) > screenSpaceError;
        }
    ...
    function executeBaseTraversal(tileset, root, screenSpaceError, frameState) {
        ...
        if (shouldVisit(tileset, root, screenSpaceError, frameState)) {
            stack.push(root);
        }
        ...
        while (stack.length > 0) {
            ...
            var traverse = (childrenLength > 0) && shouldTraverse(tileset, tile, screenSpaceError, frameState);

            if (traverse) {
                // TODO : this allows the tileset to load children before the parent is ready, particularly for additive--- is this bad? Does it matter because requests as sorted?
                // Load all children even if they are out of view. TODO : why???
                // For replacement refinement wait for all nearest descendants with content to be loaded before refining.
                // Touch children so that they are not unloaded from the cache while their siblings are loading.
                var replacementWithContent = tile.refine === Cesium3DTileRefine.REPLACE && tile.contentAvailable;
            ...
            if (traverse) {
                for (i = 0; i < childrenLength; ++i) {
                    child = children[i];
                    if (shouldVisit(tileset, child, screenSpaceError, frameState)) {
                        stack.push(child);
                    }
                }
            }
    ...
    function executeInternalBaseTraversal(tileset, root, screenSpaceError, frameState) {
        ...
        while (stack.length > 0) {
            ...
            var traverse = (tile.hasEmptyContent || tile.hasTilesetContent) && (childrenLength > 0) && shouldTraverse(tileset, tile, screenSpaceError, frameState);
